...
public class CFRDecompiler extends Decompiler {
    ...
    public void execute(File input, File output) {
        decompile(input.asJavaPath(), Map.of()).decompiled.stream().findFirst().ifPresent(code -> output.text(code.getJava()));
    }

    /**
     * Create files representing expected data, in case they do not exist yet. This is intended to
     * simplify adding new test data. If due to this new files are generated, the corresponding test
     * is marked as failed, to avoid using this setting during regular test execution by accident,
     * which would otherwise erroneously make the tests succeed.
     */
    ...
    /**
     * Updates the expected data, in case it does not match the actual data. This is intended to
     * simplify adjusting the expected output in case a decompiler change affects multiple tests. If
     * due to this the expected data is updated, the corresponding test is marked as failed, to
     * avoid using this setting during regular test execution by accident, which would otherwise
    ...
    /**
     * Path of the directory containing all test data. Directory might not exist or might be empty
     * in case the Git submodule was not cloned. In this case the test should fail. This field
     * should not be accessed directly; instead {@link #getTestDataSubDir(String)} should be used.
     * <p>
     * <b>Important:</b> This is a separate directory and not part of the test class path to avoid
     * that the test data class files are loaded by accident and interfere with the test execution.
     */
    ...
    private static Path getTestDataSubDir(String subDirPath) throws Exception {
        ...
        if (!Files.isDirectory(path)) {
            throw new ExtensionConfigurationException("Directory '" + path + "' does not exist; make sure " + "that the test data Git submodule has been cloned properly.");
        }

        try (DirectoryStream<Path> dirStream = Files.newDirectoryStream(path)) {
            if (!dirStream.iterator().hasNext()) {
                throw new ExtensionConfigurationException("Directory '" + path + "' is empty; make sure " + "that the test data Git submodule has been cloned properly.");
            }
        }
    ...
    static class ClassFileTestDataProvider implements ArgumentsProvider, AnnotationConsumer<ClassFileTestDataSource> {
        ...
        public Stream<? extends Arguments> provideArguments(ExtensionContext context) throws Exception {
            ...
            /*
             * discover tests we want to do from config file; this is expected to match the binary
    ...
    private static Map<String, String> createOptionsMap(Map<String, String> baseOptions) {
        ...
        // Do not include CFR version, would otherwise cause source changes when switching CFR
        // version
    ...
    private static DecompilationResult decompile(Path path, Map<String, String> options) {
        ...
        Sink<String> summarySink = summaryOutput::append; // Messages include line terminator,
        ...
        Sink<ExceptionMessage> exceptionSink = exceptionMessage -> {
            ...
            exceptionsOutput.append(exception.getClass().getName()).append(": ").append(exception.getMessage()).append("\n\n");
        };
        ...
        String summary = summaryOutput.toString()
                .replace(CfrVersionInfo.VERSION_INFO, "<version>")
    ...
    private static class DiffCodeResult {
        ...
        /**
         * Indicates whether due to #UPDATE_EXPECTED_DATA the expected test data was updated because
         * it differed from the actual test data.
         */
        public final boolean updatedExpectedData;

        /**
         * Whether updating the expected test data was not possible because it contains
     ...
     *
     * @param filesWithDecompilationNotes List of expected Java files which could not be updated
     *            because they contain decompilation notes
    ...
    /**
     * Fail the test because expected data could not be updated because it contains decompilation
    ...
    /**
     * Asserts that the content of the file is equal to the {@code actualData}. Returns {@code true}
     * if they are equal. Otherwise, if {@link #UPDATE_EXPECTED_DATA} is {@code true} the expected
     * data file is updated and {@code false} is returned; otherwise the corresponding
    ...
    static void assertClassFile(Path classFilePath, Map<String, String> baseOptions, Path outputDir, String filePrefix) throws IOException {
        ...
        if (!Files.exists(expectedJavaPath)) {
            ...
            assert Locator.file(expectedJavaPath).text().equals(actualJavaCode);
        }
